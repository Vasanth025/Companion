  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.data && res.data.token) {
        ...
        toast.error(res.data.message || "Invalid Login Details");
        dispatch({ type: "LOGIN_ERROR", value: res.data.message || "Login failed" });
      }
    } catch (err: any) {
      console.error(err);
      toast.error(err.response?.data?.message || "Login failed");
      dispatch({ type: "LOGIN_ERROR", value: err.response?.data?.message || "Login error" });
    } finally {
        ...
        <div className="w-full max-w-md">
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <button
              ...
              className="w-full rounded-lg bg-[#841DED] py-3 font-semibold text-white transition hover:opacity-90 cursor-pointer"
            >
          ...
          <div className="mt-4 text-center">
            <p>
              ...
              <span
                className="text-[#841DED] cursor-pointer font-semibold"
